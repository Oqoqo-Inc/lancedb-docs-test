...
<Note title="OAuth / Bearer Token Authentication">
For advanced authentication scenarios (SSO, service-to-service with rotating credentials), both Python and TypeScript SDKs support custom header providers including built-in OAuth support. See the [REST API Authentication](/api-reference/rest#oauth--bearer-token-authentication) docs for details.
</Note>
