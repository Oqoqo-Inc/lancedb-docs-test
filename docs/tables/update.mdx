...
<Note title="Partial column updates">
`when_matched_update_all()` only updates the columns present in the incoming data — other columns are left unchanged. For newly inserted rows (unmatched), any missing columns will be `null`.
</Note>
