...
<Warning title="drop_columns is a Metadata-Only Operation">
`drop_columns` updates the table schema but does **not** delete the underlying data from disk. The column data remains in `.lance` files and is still accessible via older table versions.

To fully remove the data from storage, run `compact_files` followed by `cleanup_old_versions` after dropping the column:

```python
table.drop_columns(["sensitive_column"])
table.compact_files()
