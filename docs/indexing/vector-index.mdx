...
<Note title="LanceDB Cloud: IVF_HNSW_PQ Not Supported">
`IVF_HNSW_PQ` is not supported in LanceDB Cloud. Attempting to create an `IVF_HNSW_PQ` index on a Cloud table will raise a `ValueError`. Use `IVF_HNSW_SQ` instead, which provides a strong recall/latency trade-off and is fully supported in LanceDB Cloud.
</Note>
